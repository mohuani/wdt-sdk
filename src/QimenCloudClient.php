...
namespace App\Services\Wdt;

use Exception;

class QimenCloudClient {

    private const TOP_SDK_WORK_DIR = '/tmp/';
    ...
    protected function logCommunicationError ($apiName, $requestUrl, $errorCode, $responseTxt) {
        ...
        $logger->conf["log_file"] = rtrim(self::TOP_SDK_WORK_DIR, '\\/').'/'."logs/top_comm_err_".$this->appkey."_".date("Y-m-d").".log";
    ...
    public function execute ($request, $session = null, $bestUrl = null) {
        ...
        if (isset($respObject->code)) {
            $logger = new TopLogger;
            $logger->conf["log_file"] = rtrim(self::TOP_SDK_WORK_DIR, '\\/').'/'."logs/top_biz_err_".$this->appkey."_".date("Y-m-d").".log";
            $logger->log([
                date("Y-m-d H:i:s"),
                $resp,
            ]);
